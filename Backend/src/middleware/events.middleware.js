...
myEvent.on(EventName.RESEND_ACTIVATION, async (data) => {
    try {
        await Mailsvc.mailSend({
            to: data.email,
            sub: "Resend Activation",
            message: `
            Dear ${data.name},</br>
            <p> A new activation token has been sent. please click the link below or redirect to the link given below
            <a href="${process.env.FRONTEND_URL}/activate/${data.token}"> ${process.env.FRONTEND_URL}/activate/${data.token}</a></p>
            </br>
            <p><strong>Note:</strong>Please do not reply to this email.</p>
            <p>Regards</p>
