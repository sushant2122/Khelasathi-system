  ...
  */
    getAdminStats = async (req, res, next) => {
        try {
            ...
            const totalRefund = await Transaction.sum('total_payment', {
                where: {
                    payment_status: 'refunded'
                }
            });

            const totalRefundfailed = await Transaction.sum('total_payment', {
                where: {
                    payment_status: 'refund-failed'
                }
            });

            const total = totalRevenue - (totalRefund + totalRefundfailed)
